Attribute VB_Name = "MxXls_Ws_Idx"
Option Compare Text
Option Explicit
Const CMod$ = "MxXls_Ws_Idx."

Sub AddWsIdx(B As Workbook)
...
Dim W$():             W = AeEle(Wny(WbRg(At)), WsRg(At).Name) ' All other wsn ept the-ws-of-@At
...
Dim Sq():          Sq = R.Value
If Not IsEmpSq(Sq) Then Debug.Print "Som cell has value in Rg[" & R.Address & "]": Exit Sub 'Exit=>
...
Dim Wsn: For Each Wsn In W
    If Not IsEmpty(R.Value) Then ThwImposs CSub ' The IsVdt is not OK
        R.Value = Wsn              ' <== Set Wsn
          Set R = RgRC(R, 1, 0)
        R.Value = "Go"             ' <== Set Go
            Stop '
