Attribute VB_Name = "MxVb_Ay_Op_Push"
Option Compare Text
Option Explicit
Const CMod$ = "MxVb_Ay_Push."

Function EleShf(OAy)
EleShf = OAy(0)
OAy = AeFst(OAy)
End Function

Function EleShfIf(OAy)
If Not IsEmpAy(OAy) Then EleShfIf = EleShf(OAy)
End Function

Sub PushNDupDy(ODy(), Dr)
If HasDr(ODy, Dr) Then Exit Sub
PushI ODy, Dr
End Sub

Sub Push(O, M)
Dim N&: N = Si(O)
ReDim Preserve O(N)
If IsObject(M) Then
    Set O(N) = M
Else
    O(N) = M
End If
End Sub

Sub PushAp(O, ParamArray Ap())
Dim Av(), I: Av = Ap
For Each I In Av
    Push O, I
Next
End Sub

Sub PushAy(O, Ay)
Dim I: For Each I In Itr(Ay)
    Push O, I
Next
End Sub

Sub PushAyNoDup(O, Ay)
Dim I: For Each I In Itr(Ay)
    PushNoDup O, I
Next
End Sub
Sub PushS(O$(), S$): PushI O, S: End Sub

Sub PushStropt(OSy$(), M As Stropt)
If M.Som Then PushI OSy, M.Str
End Sub
Sub PushSs(O$(), Ss$): PushIAy O, SySs(Ss): End Sub
Sub PushI(O, M)
Dim N&: N = Si(O)
ReDim Preserve O(N)
O(N) = M
End Sub

Sub PushIx(O&(), I&)
If I >= 0 Then PushI O, I
End Sub

Sub PushSy(O$(), Sy$()): PushIAy O, Sy: End Sub

Sub PushIAy(O, Ay)
Const CSub$ = CMod & "PushIAy"
ChkIsAy Ay, CSub
Dim M: For Each M In Itr(Ay)
    PushI O, M
Next
End Sub

Sub PushSomSi(OAy, IAy) ' If @IAy has element, Push it to @OAy, else :nop
'nop:: #No-operation#
If Si(IAy) = 0 Then Exit Sub
PushI OAy, IAy
End Sub

Sub PushItmAy(O, Itm, Ay)
Push O, Itm
PushAy O, Ay
End Sub

Sub PushNoDup(O, M)
If NoEle(O, M) Then PushI O, M
End Sub

Sub PushNoDupNB(O, M)
If M = "" Then Exit Sub
If Not HasEle(O, M) Then PushI O, M
End Sub

Sub PushNoDupDr(ODy(), Dr)
Dim IDr
For Each IDr In Itr(ODy)
    If IsEqAy(IDr, Dr) Then Exit Sub
Next
PushI ODy, Dr
End Sub

Sub PushNoDupIAy(O, Ay)
Dim I: For Each I In Itr(Ay)
    PushNoDup O, I
Next
End Sub

Sub PushNB(O$(), M)
If Trim(RplCrLf(M)) <> "" Then
    PushI O, M
End If
End Sub

Sub PushNBAy(O$(), Ay)
Dim I: For Each I In Itr(Ay)
    PushNB O, I
Next
End Sub

Sub PushNBNDup(O$(), S$)
If Trim(S) <> "" Then If NoEle(O, S) Then PushI O, S
End Sub

Sub PushNonEmp(O, M)
If Not IsEmp(M) Then Push O, M
End Sub

Sub PushNonNothing(O, M)
If IsNothing(M) Then Exit Sub
PushObj O, M
End Sub

Sub PushExcNothing(O, M)
If IsNothing(M) Then PushObj O, M
End Sub

Sub PushObj(O, M)
Dim N&: N = Si(O)
ReDim Preserve O(N)
Set O(N) = M
End Sub

Sub PushObjItr(O, Itr)
Dim Obj: For Each Obj In Itr
    PushObj O, Obj
Next
End Sub

Sub PushObjAy(O, Oy)
Dim I: For Each I In Itr(Oy)
    PushObj O, I
Next
End Sub

Function Si&(A): On Error Resume Next: Si = UBound(A) + 1: End Function
Function UB&(A): UB = Si(A) - 1: End Function
Function Pop(O)
Asg EleLas(O), _
    Pop
If Si(O) = 1 Then
    Erase O
Else
    ReDim Preserve O(UB(O) - 1)
End If
End Function

Function PopI(O)
PopI = EleLas(O)
If Si(O) = 1 Then
    Erase O
Else
    ReDim Preserve O(UB(O) - 1)
End If
End Function

Function AyReDim(Ay, U)
Dim O: O = Ay
If U < 0 Then
    Erase O
Else
    ReDim Preserve O(U)
End If
AyReDim = O
End Function
